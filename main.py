...
class player(object):
    def __init__(self, x, y, width, height):
        ...
        self.walkCount = 0
        self.standing = True
        self.hitbox = (self.x + 17, self.y + 11, 29,
                       52)  # we have a tuple with 4 things, its the rectangle which goes around our character
    ...
    def draw(self, win):
        ...
        self.hitbox = (self.x + 17, self.y + 11, 29,
                       52)  # we have to draw it everytime that we draw the character, so this is why we put it here
...
font = pygame.font.SysFont('comicsans', 30,
                           True)  # we want to write a text with the font Comicsans, with size 30, we want bold = True
...
while run:
    ...
    if goblin.visible == True:
        if man.hitbox[1] < goblin.hitbox[1] + goblin.hitbox[3] and man.hitbox[1] + man.hitbox[3] > goblin.hitbox[
            1]:  # if bullet is between the y corners
    ...
    for bullet in bullets:  # a list of bullets defined above, if they are in the screen (less than 500 and more than 0)
        ...
        if bullet.y - bullet.radius < goblin.hitbox[1] + goblin.hitbox[3] and bullet.y + bullet.radius > goblin.hitbox[
            ...
            if bullet.x + bullet.radius > goblin.hitbox[0] and bullet.x - bullet.radius < goblin.hitbox[0] + \
                    goblin.hitbox[2]:  # if bullet is between the x corners
                hitSound.play()
                goblin.hit()
                score += 1
        ...
        if bullet.x < 500 and bullet.x > 0:
            ...
            bullets.pop(bullets.index(
                bullet))  # pop means remove an element of the list, so its gonna find the index of the bullet and remove it from the list bullets
    ...
    if keys[pygame.K_SPACE] and shootLoop == 0:
        ...
        if len(bullets) < 5:  # this is the max of the bullets that the man can shoot at the same time
            bullets.append(projectile(round(man.x + man.width // 2), round(man.y + man.height // 2), 6, (0, 0, 0),
                                      facing))  # man.with//2 its gonna shoot from the middle of the man
        shootLoop = 1
    ...
    if not (man.isJump):
        ...
        if man.jumpCount >= -10:
            ...
            if man.jumpCount < 0:  # if the jump is after the max level, the character should go down, so the y should de increase and be negative
                neg = -1
            man.y -= (
                                 man.jumpCount ** 2) / 2 * neg  # we made a square function and we are gonna be slowing it down 1 by 1 (next line of code)
